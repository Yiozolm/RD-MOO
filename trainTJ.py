...
def train_one_epoch(model, criterion, train_dataloader, optimizer, aux_optimizer, train_step, tb_writer=None,
    ...
    if trajectory_state is None:
        trajectory_state = BalancedRDTrajectoryState(device=next(model.parameters()).device)
    ...
    for x in train_dataloader:
        ...
        # ----------- Coarse-to-Fine Balanced R-D Gradient Step ----------- #
        # (1) Compute bpp_loss and mse_loss separately
        loss_rate = out_criterion["bpp_loss"]
        loss_dist = out_criterion["distortion"]
        eps = 1e-8

        # (2) Get gradients for rate
        optimizer.zero_grad()
        ...
        # (5) Set combined gradients to model parameters
